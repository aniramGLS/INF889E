...
def run_MLP(X_train, y_train, X_test, y_test): ### INITIALISATION DU MODELE
    ...
    model = MLPClassifier(solver='sgd', hidden_layer_sizes=(3, 3), activation='identity', max_iter=1000,
                           learning_rate = 'adaptive', early_stopping = True, alpha= 1e-7, random_state=26)  # CREATION DU MODELE MLP
    ...
    figure = ConfusionMatrixDisplay(confusion_matrix=c_matrix, display_labels=model.classes_, xticks_rotation = 50)  
    figure.plot()   
